...
class ColorDialog extends JDialog implements ActionListener {
	...
	public static Color getColor(Frame frame, Color color, boolean useCapture){
		captureThread = null;
		new ColorDialog(frame, color, useCapture);
	...
	private ColorDialog(Frame frame, Color color, boolean useCapture){
		super(frame, true);
		...
		if(useCapture){
			JButton btn2 = new JButton(International.getDialogText("Choose from Screen"));
...
class CaptureThread extends Thread {
	...
	public void run(){
		...
		if(clicked){
			ColorDialog.chooser.setColor(color);
